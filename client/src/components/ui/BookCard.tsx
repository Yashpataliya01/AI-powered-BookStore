    ...
    return (
      <div className="group relative bg-[var(--bg-secondary)] rounded-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-8 flex flex-col justify-between">
          <div>
            <p className="text-xs uppercase tracking-widest text-[var(--text-muted)] font-medium mb-3">{book.category}</p>
            <Link to={`/book/${book.id}`}>
              <h3 className="font-['Playfair_Display'] text-3xl font-bold text-[var(--text-primary)] leading-tight hover:text-[var(--accent)] transition-colors mb-2">
                {book.title}
              </h3>
            </Link>
            ...
            <p className="text-sm text-[var(--text-secondary)] leading-relaxed line-clamp-3 mt-4">{book.description}</p>
          </div>
          <div className="flex items-center justify-between mt-6">
            <div className="flex items-baseline gap-2">
              <span className="font-['Playfair_Display'] text-2xl font-bold text-[var(--text-primary)]">${book.price}</span>
              {book.originalPrice && <span className="text-sm text-[var(--text-muted)] line-through">${book.originalPrice}</span>}
            </div>
            <button onClick={() => dispatch(addToCart(book))}
              className={`px-6 py-2.5 rounded-full text-sm font-semibold transition-all ${inCart ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-[var(--accent)] text-white hover:bg-[var(--accent-hover)]'}`}>
    ...
    <div className="group relative flex flex-col">
      ...
      <div className="relative overflow-hidden rounded-xl bg-[var(--bg-secondary)] aspect-[3/4]">
        ...
        <button onClick={() => dispatch(toggleWishlist(book))}
          className="absolute top-3 right-3 w-7 h-7 rounded-full bg-[var(--bg-primary)]/90 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <svg width="13" height="13" viewBox="0 0 24 24"
            fill={wishlisted ? 'var(--accent)' : 'none'}
            stroke={wishlisted ? 'var(--accent)' : 'currentColor'}
            strokeWidth="2" className="text-[var(--text-primary)]">
        ...
        <div className="absolute bottom-0 inset-x-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
          <button onClick={() => dispatch(addToCart(book))}
            className={`w-full py-2 rounded-full text-xs font-semibold transition-colors ${inCart
      ...
      <div className="mt-3 flex-1">
        <p className="text-[10px] uppercase tracking-widest text-[var(--text-muted)] font-medium">{book.category}</p>
        <Link to={`/book/${book.id}`}>
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-baseline gap-1.5">
            <span className="text-sm font-bold text-[var(--text-primary)]">${book.price}</span>
            {book.originalPrice && <span className="text-xs text-[var(--text-muted)] line-through">${book.originalPrice}</span>}
          </div>
