...
import { useAppDispatch, useCartCount, useUI, useIsAuthed, useCurrentUser } from '@/hooks';
...
const Navbar: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[var(--bg-primary)] shadow-sm border-b border-[var(--border)]' : 'bg-transparent'}`}>
      <nav className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isAuthed && user ? (
            <div className="relative hidden md:block">
              <button onClick={() => setUserMenuOpen(o => !o)}
                ...
                <span className="text-xs font-medium text-[var(--text-secondary)] max-w-[80px] truncate">{user.name.split(' ')[0]}</span>
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl shadow-xl z-20 py-1 overflow-hidden">
          ...
          <button onClick={() => dispatch(toggleMobileMenu())}
            ...
            <span className={`block w-5 h-px bg-[var(--text-primary)] transition-all duration-300 ${mobileMenuOpen ? 'rotate-45 translate-y-[3px]' : ''}`} />
            <span className={`block w-5 h-px bg-[var(--text-primary)] transition-all duration-300 ${mobileMenuOpen ? '-rotate-45 -translate-y-[3px]' : ''}`} />
          </button>
        </div>
      </nav>
      ...
      {/* Mobile menu */}
      <div className={`md:hidden transition-all duration-300 overflow-hidden bg-[var(--bg-primary)] border-b border-[var(--border)] ${mobileMenuOpen ? 'max-h-80 opacity-100' : 'max-h-0 opacity-0'}`}>
